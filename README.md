# Current gameplay: FFA Deathmatch

The playable mode is now **FFA only**. Gem Grab, training, and bot battles are temporarily disabled. All connections join one shared `arena`. Four seasonal preview cards (Spring, Summer, Fall, Winter) switch the map immediately for everyone. The header selector also works during active matches. Changing season preserves the timer, scores, health, cooldowns and death state, clears in-flight attacks, and moves players only if the new terrain blocks their position. Late joiners load the active map. Legacy URL room/mode/map parameters do not split the room.

- Five-minute rounds; most kills wins. Ties use fewer deaths, then stable player ID. The arena waits for anyone to press START GAME. Combat stops at the deadline or when anyone presses END GAME; the next round requires START GAME again.
- WASD/arrow keys move in screen directions, including normalized diagonals. Continuous circle collision and substeps prevent crossing trees, water, or bounds. Space/2 dashes with a fading smoke trail.
- Click/1 fires Piercing Thrust toward the cursor, hitting each opponent along its path once. Q/E/3 marks a circular area for one second, then detonates for 45% of base HP with a six-stage explosion.
- Death disables control and hides the avatar. Respawn takes five seconds, followed by a two-second shield. Health does not regenerate; respawning restores full HP.
- `localStorage.player_profile` stores nickname and outfit. Returning players enter directly; the character menu offers Auto-Mix, Reset to Default (bare feet, no hat/accessories), and profile editing. Cosmetics and nickname survive respawns and rounds; match scores reset each round.
- Sound defaults off and persists in `localStorage.game_sound`; enabled audio resumes on the first browser gesture. Local damage plays a synthesized “ahh”, and successful hits 2–5 play escalating combo cues. The hit streak resets on death or match end.
- Players can walk before and between matches; attacks require an active round. The leaderboard toggles with the top-right button or Escape and highlights the top three. End-of-round MVP uses `100 × kills + 0.1 × actual damage dealt − 25 × deaths`, with ties resolved by kills, fewer deaths, damage, then player ID. Only countdown expiry shows its centered three-second celebration; manual End Game is quiet; scores and timing are tunable in `gameConfig.js`.
- All four maps preserve the blocky geometry, flat tiles, and tiered water with seasonal palettes: green spring, sandy summer with sandstone cover, amber fall foliage, and snowy winter pines. Seeded layouts vary tree clusters, cottage footprints, raised solid mountain terraces and paths. Spring has a northern stream; Summer has a western oasis and sandstone ridge; Fall has a mid-map river with broad timber bridges; Winter has northern meltwater and snowy cliffs. The raised cliffs and houses block movement; bridges are walkable. Regenerate the layout matrices with `python3 scripts/generate-arenas.py`.
- The arena cursor is 24 pixels and changes to a sharp pink arrow over an enemy.
- Movement uses physical key codes and clears input on IME composition, right-click/context menu, blur, visibility change, and editable-field focus.
- Balance values live in `src/gameConfig.js`. `npm run dev` starts the client and authoritative server, `npm test` checks gameplay/transport/rendering helpers, and `npm run build` produces the client build.

The documentation below describes the earlier prototype and includes temporarily disabled modes.
