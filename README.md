...
- WASD/arrow keys move in screen directions, including normalized diagonals. Continuous circle collision and substeps prevent crossing trees, water, or bounds. Space/2 dashes.
...
- Death disables control and hides the avatar. Respawn takes five seconds, followed by a two-second shield. Regeneration begins after three seconds without dealing or taking damage.
...
- Players can walk before and between matches; attacks require an active round. The leaderboard toggles with the top-right button or Escape and highlights the top three. End-of-round MVP uses `100 × kills + 0.1 × actual damage dealt − 25 × deaths`, with ties resolved by kills, fewer deaths, damage, then player ID. Its centered celebration lasts three seconds; scores and timing are tunable in `gameConfig.js`.
- All four maps preserve the blocky geometry, flat tiles, and tiered water with seasonal palettes: green spring, sandy summer with sandstone cover, amber fall foliage, and snowy winter pines. Their open center ellipses have no collision obstacles; sparse cover sits outside them, and edge streams have broad path crossings. Regenerate the layout matrices with `python3 scripts/generate-arenas.py`.
